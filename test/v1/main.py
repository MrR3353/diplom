...
from etobasa import File, Tree, Commit, save, load
...
def init():
    # git init
    os.mkdir(VCS_FOLDER)
    os.mkdir(os.path.join(VCS_FOLDER, 'commits'))
...
def save_commit(new_commit):
    ...
    if not head.exists():   # создаем HEAD, записываем туда хэш коммита
        ...
        save(os.path.join(VCS_FOLDER, 'commits', new_commit.hash), new_commit)
    else:
