...
def init():
    ...
    os.makedirs(os.path.join(VCS_FOLDER, 'commits'), exist_ok=True)
...
def save_commit(new_commit):
    ...
    if not head.exists():   # создаем HEAD, записываем туда хэш коммита
        ...
        # сохраняем коммит
        os.mkdir(os.path.join(VCS_FOLDER, 'commits', new_commit.hash))
        new_commit.save(os.path.join(VCS_FOLDER, 'commits', new_commit.hash))
    else:
        with open(head, 'r') as file:
            prev_commit_hash = file.read()
        # загружаем предыдущий коммит
        prev_commit = load(os.path.join(VCS_FOLDER, 'commits', prev_commit_hash, prev_commit_hash))
